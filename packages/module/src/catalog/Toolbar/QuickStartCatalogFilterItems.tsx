...
export const QuickStartCatalogFilterSearch = ({
  searchInputText,
  handleTextChange,
  ...props
}) => {
...
export const QuickStartCatalogFilterStatusWrapper: React.FC<QuickStartCatalogFilterStatusWrapperProps> = ({
  ...
  const statusTypes = {
    [QuickStartStatus.COMPLETE]: t(
      "quickstart~Complete ({{statusCount, number}})",
      {
        statusCount: quickStartStatusCount[QuickStartStatus.COMPLETE],
      }
    ),
    [QuickStartStatus.IN_PROGRESS]: t(
      "quickstart~In progress ({{statusCount, number}})",
      {
        statusCount: quickStartStatusCount[QuickStartStatus.IN_PROGRESS],
      }
    ),
    [QuickStartStatus.NOT_STARTED]: t(
      "quickstart~Not started ({{statusCount, number}})",
      {
  ...
  const onRowfilterSelect = React.useCallback(
    (e, val) => {
      ...
      if (!equalsIgnoreOrder(statusFilters, selectedFiltersList)) {
        ...
        setSelectedFilters(
          selectedFiltersList.map((filterKey) => statusTypes[filterKey])
        );
      }
      if (useQueryParams) {
